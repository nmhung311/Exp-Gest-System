...
export default function GuestsPage(){
  ...
  const bulkCheckIn = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setCopyMessage(`Đã check-in thành công ${selectedGuests.size} khách!`)
        setCopyType('success')
        setShowPopup(true)
  ...
  const bulkCheckOut = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setCopyMessage(`Đã check-out thành công ${selectedGuests.size} khách!`)
        setCopyType('info')
        setShowPopup(true)
        setPopupVisible(true)
        clearSelection()
  ...
  const bulkDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setCopyMessage(`Đã xóa thành công ${selectedGuests.size} khách!`)
        setCopyType('success')
        setShowPopup(true)
        setPopupVisible(true)
        clearSelection()
  ...
  async function deleteGuest(guestId: number, guestName: string) {
    ...
    try {
      ...
      if (response.ok) {
        loadGuests()
        setCopyMessage("Xóa khách mời thành công!")
        setCopyType('success')
        setShowPopup(true)
        setPopupVisible(true)
        setTimeout(() => {
  ...
  async function copyInviteLink(guestId: number, guestName: string) {
    try {
      ...
      if (response.ok) {
        ...
        setCopyMessage(`Đã copy link thiệp mời cho ${guestName}!`)
        setCopyType('success')
        setShowPopup(true)
        setPopupVisible(true)
        setTimeout(() => {
        ...
        setCopyMessage("Lỗi khi tạo link thiệp mời")
        setCopyType('error')
        setShowPopup(true)
        setPopupVisible(true)
        setTimeout(() => {
      ...
      setCopyMessage("Lỗi khi copy link thiệp mời")
      setCopyType('error')
      setShowPopup(true)
      setPopupVisible(true)
      setTimeout(() => {
  ...
  const exportGuests = async (guestsToExport: Guest[], filename: string) => {
    try {
      ...
      setCopyType('success')
      setCopyMessage(`Đã xuất thành công ${guestsToExport.length} khách!`)
      setShowPopup(true)
      setPopupVisible(true)
      setTimeout(() => {
        setPopupVisible(false)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-400 via-purple-500 to-cyan-400 text-transparent bg-clip-text">Quản lý khách mời</h1>
        <div className="flex gap-2">
          ...
          <button 
            onClick={loadGuests}
            className="group relative px-4 py-2 bg-gradient-to-r from-gray-500/20 to-slate-500/20 border border-gray-500/30 text-gray-400 rounded-lg hover:from-gray-500/30 hover:to-slate-500/30 hover:border-gray-400/50 transition-all duration-300 flex items-center gap-2 backdrop-blur-sm hover:shadow-lg hover:shadow-gray-500/20"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-right">
                ...
                <div className="text-sm text-cyan-300/80 font-medium">Tổng khách mời</div>
              </div>
            </div>
        ...
        {/* Pending Card */}
        <div className="group relative bg-gradient-to-br from-yellow-500/10 to-orange-500/10 backdrop-blur-sm border border-yellow-500/20 rounded-2xl p-6 hover:from-yellow-500/20 hover:to-orange-500/20 hover:border-yellow-400/40 transition-all duration-300 hover:shadow-lg hover:shadow-yellow-500/20">
        ...
        <div className="group relative bg-gradient-to-br from-green-500/10 to-emerald-500/10 backdrop-blur-sm border border-green-500/20 rounded-2xl p-6 hover:from-green-500/20 hover:to-emerald-500/20 hover:border-green-400/40 transition-all duration-300 hover:shadow-lg hover:shadow-green-500/20">
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-right">
                <div className="text-3xl font-bold text-white mb-1">{stats.accepted}</div>
        ...
        {/* Declined Card */}
        <div className="group relative bg-gradient-to-br from-red-500/10 to-pink-500/10 backdrop-blur-sm border border-red-500/20 rounded-2xl p-6 hover:from-red-500/20 hover:to-pink-500/20 hover:border-red-400/40 transition-all duration-300 hover:shadow-lg hover:shadow-red-500/20">
      ...
      {showImportModal && (
        <Portal>
          <div className="fixed inset-0 h-[100dvh] w-[100dvw] z-[9998] flex items-center justify-center p-4">
            ...
            <div className="relative bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-2xl max-h-[90dvh] overflow-y-auto">
            ...
            {importType === "json" && (
              <div className="space-y-3 mb-6">
                <p className="text-sm text-white/60">Nhập JSON array của khách mời hoặc upload file JSON</p>
                ...
                <div className="mb-4">
                  <div className="flex gap-2">
                    <input 
                      ...
                      className="flex-1 bg-black/30 border border-white/20 rounded-xl p-3 text-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-purple-500 file:text-white hover:file:bg-purple-600" 
                    />
                    <button
                      type="button"
                      onClick={() => {
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    className="w-full h-40 bg-black/30 border border-white/20 rounded-xl p-3 text-white placeholder-white/50 font-mono text-sm" 
                    value={text} 
            ...
            {importType === "csv" && (
              <div className="space-y-3 mb-6">
                <p className="text-sm text-white/60">Chọn file CSV với các cột: title, name, role, organization, tag, email, phone</p>
                <input 
                  type="file" 
                  id="csvFile" 
                  accept=".csv" 
                  className="w-full bg-black/30 border border-white/20 rounded-xl p-3 text-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-500 file:text-white hover:file:bg-blue-600" 
                />
...
Ms,Tên khách 2,Manager,Công ty XYZ,Tag2,email2@example.com,0900000001</pre>
            ...
            {result && (
              <div className="mt-6 p-4 bg-black/30 border border-white/20 rounded-xl">
      ...
      {showPopup && (
        <div className={`fixed top-4 right-4 z-[9999] transform transition-all duration-300 ease-out ${
          ...
          <div className={`px-4 py-3 rounded-2xl shadow-2xl max-w-xs backdrop-blur-md border ${
            ...
            <div className="flex items-center gap-3">
              <div className="flex-shrink-0">
                ...
                {copyType === 'warning' && (
                  <div className="w-8 h-8 bg-amber-500/20 rounded-xl flex items-center justify-center border border-amber-400/30">
      ...
      {showExportPopup && (
        <Portal>
          <div className="fixed inset-0 h-[100dvh] w-[100dvw] bg-black/50 backdrop-blur-sm flex items-center justify-center z-[9998]">
            <div className="bg-gradient-to-br from-blue-500/20 to-purple-500/20 border border-blue-500/30 rounded-2xl p-6 backdrop-blur-sm max-w-md w-full mx-4 shadow-2xl">
