  ...
  const deleteEvent = async (eventId: number, eventName: string) => {
    if (confirm(`Bạn có chắc chắn muốn xóa sự kiện "${eventName}"?`)) {
      try {
        ...
        await loadEvents()
        setToastMsg(`Đã xóa sự kiện "${eventName}"`) 
        setToastType('success')
        setToastVisible(true)
        setTimeout(() => setToastVisible(false), 2000)
      } catch (error) {
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10">
                <th className="text-left py-4 px-4 text-white/80 font-medium">Tên sự kiện</th>
                <th className="text-left py-4 px-4 text-white/80 font-medium">Ngày & Giờ</th>
                <th className="text-left py-4 px-4 text-white/80 font-medium">Địa điểm</th>
                <th className="text-left py-4 px-4 text-white/80 font-medium">Số khách tối đa</th>
                <th className="text-left py-4 px-4 text-white/80 font-medium">Trạng thái</th>
                <th className="text-left py-4 px-4 text-white/80 font-medium">Thao tác</th>
              </tr>
            </thead>
            <tbody>
              {paginatedEvents.map((event) => (
                <tr key={event.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                  ...
                  <td className="py-4 px-4 text-white/80">{event.location}</td>
                  <td className="py-4 px-4 text-white/80">{event.max_guests}</td>
                  <td className="py-4 px-4">
